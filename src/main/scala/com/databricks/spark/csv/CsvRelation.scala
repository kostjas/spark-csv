...
case class CsvRelation protected[spark] (
    ...
    fieldsTypeMap: Map[String, DataType] = Map.empty)(@transient val sqlContext: SQLContext)
  ...
  private def inferSchema(): StructType = {
    if (this.userSchema != null) {
      ...
      val schemaFields = header.map { fieldName =>
        StructField(fieldName.toString, fieldsTypeMap.getOrElse(fieldName, StringType), nullable = true)
      }
