...
class CsvSuite extends FunSuite {
  ...
  test("DSL test typed columns using sparkContext.csvFile") {
    ...
    val results = TestSQLContext.csvFile(carsTypedColumnsFile, fieldsTypeMap = typedColumnsMap)
  ...
  test("Expect parsing error with wrong type for FailFast mode using sparkContext.csvFile") {
    ...
    intercept[SparkException] {
      TestSQLContext.csvFile(carsTypedColumnsFailFile, fieldsTypeMap = typedColumnsMap, mode = "FAILFAST").collect()
    }
  }
